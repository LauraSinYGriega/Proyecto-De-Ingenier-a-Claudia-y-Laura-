...
public class Recolecta_parametros_clase implements Runnable {

	private int repeticiones;//función para poder utilizar la variable repeticiones y también usarla en el main

	public Recolecta_parametros_clase (int repeticiones) {
	...
	public void run() {
		for (int i=0; i<repeticiones;i++) {
			try {
				Thread.sleep(1000);
				try (BufferedWriter bw = new BufferedWriter(new FileWriter("BaseDatos_v8.csv",true))) {
					File fichero = new File ("BaseDatos_v8.csv");
					if (!fichero.exists()) {
